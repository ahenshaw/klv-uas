...
impl KlvPacket {
    ...
    fn get_ber_value(buf: &mut Cursor<Box<[u8]>>) -> usize {
        ...
        let msb = bits
            .get(0)
            .expect("Cannot get the first bit from the byte array");
    ...
    fn calculate_checksum(buf: &[u8]) -> u16 {
        ...
        buf[..buf.len()].iter().enumerate().for_each(|(idx, byte)| {
            bcc = bcc.wrapping_add((*byte as u16) << (8 * ((idx + 1) % 2)) as u16)
        });
        return bcc;
    }
    ...
    pub fn from_bytes(bytes: Box<[u8]>) -> Result<KlvPacket, Errors> {
        ...
        let calculated_checksum =
            Self::calculate_checksum(bytes.get(start_index..checksum_bytes_length).unwrap());
...
mod tests {
    ...
    fn packet_from_value(test_value: Vec<u8>) -> Vec<u8> {
        ...
        let length: Vec<u8> = vec![(precision_timestamp_bytes.len() + test_value.len() + 4)
            .try_into()
